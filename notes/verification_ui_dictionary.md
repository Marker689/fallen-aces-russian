...
## ⚠️ КРИТИЧНОЕ дополнение после верификации (2026-08-14)

**Найден баг, пропущенный верификатором:** строки с rich-text тегами `<color=...>`
(содержат `=`) игнорируются XUnity.AutoTranslator.

Проверено по исходникам плагина (`TextHelper.ReadTranslationLineAndDecode`,
`TextTranslationCache.LoadTranslationsInStream`):
- Каждая строка словаря должна содержать **ровно один неэкранированный `=`**
  (разделитель `original=translation`).
- **Второй неэкранированный `=` → строка целиком игнорируется** (`return null`).
- Экранирование литерального `=` в ключе/значении — через `\=` (декодируется
  обратно в `=` при загрузке; матчинг идёт по раскодированной строке).

**Затронуты 12 строк** (меню-подсказки с `<color=#C42620>`, `<color=red>`,
журнал/карта `<color=white>`/`<color=#4BB740>`): 57, 58, 81, 226–234.
До исправления они молча не переводились. Верификатор дал по ним «PASS» из-за
наивного сплита по первому `=` — это была ложная положительная оценка.

**Исправление:** во всех 12 строках внутренние `=` экранированы как `\=`
(сохранён ровно один неэкранированный `=`-разделитель). После декодирования ключи
совпадают с реальными строками игры. Повторная проверка: 216 записей, 0
мальформированных.

**Вывод по качеству:** техническая корректность словаря НЕ была «PASS» — после
фикса она действительно PASS. Прочие выводы верификатора (глоссарий, стиль,
консистентность) остаются в силе. Рекомендуется обновить инструмент проверки
словаря, чтобы ловить строки с >1 неэкранированным `=`.
